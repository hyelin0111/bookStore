...
<div>
<!-- 통합검색, 책제목, 저자명, 출판사명 -->
<%=type%> <span id="searchData2">'<%=searchBook %>'</span> 검색결과  <br />
<hr /> <br />
<form>
<table id="new_book2">
   <tr>
      <th colspan="6">
         <nav id="search_nav2">
            <a href="<%=request.getContextPath()%>/sort/sellAmount?searchBook=<%=searchBook%>&type=<%=type%>&cPage=<%=request.getAttribute("cPage")%>">판매량</a><span class="sep"> | </span>
            <!-- <a href="#">신상품</a><span class="sep"> | </span> -->
            <a href="<%=request.getContextPath()%>/sort/rowPrice?searchBook=<%=searchBook%>&type=<%=type%>&cPage=<%=request.getAttribute("cPage")%>">낮은가격</a><span class="sep"> | </span>
            <a href="<%=request.getContextPath()%>/sort/highPrice?searchBook=<%=searchBook%>&type=<%=type%>&cPage=<%=request.getAttribute("cPage")%>">높은가격</a>
         </nav>
      </th>
   </tr>
