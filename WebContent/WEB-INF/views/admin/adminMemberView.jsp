...
<%
	...
	String birthday = member.getMember_birthday();
    String email = member.getMember_email()!=null?member.getMember_email():"";
    
    System.out.println("dasfsdfsadf  =  "+member.getMember_status());
...
<div id="h-container1">
	<h2 class="member_update1">Admin - 회원 정보 수정</h2>
	<hr id="line3" />
</div>
<br />

<form action="#"id="memberFrm" method="post" name="memberFrm"  onsubmit="return fn_update_validate()"> 
<div id="table-container1">
	...
	<tr height="60">
		...
		<td width="600">  
			<%if("Y".equals(member.getMember_status())){  %>
				<input type="radio" name="status" id="deleteY" value="Y"  checked>
				<label for="deleteY">활동중</label>&nbsp;
				<input type="radio" name="status" id="deleteN" value="N" >
				<label for="deleteN">탈퇴</label>
			<%} else {%>
				<input type="radio" name="status" id="deleteY" value="Y"  >
				<label for="deleteY">활동중</label>&nbsp;
				<input type="radio" name="status" id="deleteN" value="N" checked>
				<label for="deleteN">탈퇴</label>
			<%} %>
	 	</td>
 	</tr>
 	<tr height="60">
		...
		<td width="600">  
			<input type="text" name="memId" id="memId3" size="16" value="<%= member.getMember_id() %>"  readonly>
	 	</td>
 	</tr>
 	<tr height="60">
 		<td id="title3" >
		...
		<td width="600">  
			<input type="text" name="password" id="pwd3" size="16"  required="required">
	 	</td>
 	</tr>
 	<tr height="60">
 		<td id="title3" >
		...
		<td width="600">  
			<input type="text"id="password1" size="16" >
	 	</td>
 	</tr>
 	...
 	<tr height="40">
		...
		<%if("M".equals(member.getMember_gender())){  %>
			<input type="radio" name="gender" id="genderM" value="M" checked disabled>
			<label for="genderM">남자</label>&nbsp;
			<input type="radio" name="gender" id="genderF"value="F" disabled>
			<label for="genderF">여자</label>
		<%}else{ %>
			<input type="radio" name="gender" id="genderM" value="M" disabled>
			<label for="genderM">남자</label>&nbsp;
			<input type="radio" name="gender" id="genderF"value="F" checked disabled>
			<label for="genderF">여자</label>
		<%} %>
		</td>
	</tr> 
...
<tr height="40">
  ...
  <td>
	...
	<div class="btn-area1">
		 ...
		 <input type="button" id="btn-delete1" value="회원삭제" onclick="fn_delete_member();"/>
		<input type="button" id="btn-cancel1" onclick="location.href='<%=request.getContextPath()%>/admin/adminMemberListView';" value="취소"/>
	</div>
</form>
<script>
...
function fn_update_member(){
	var frm = $("#memberFrm");
	var url = "<%=request.getContextPath() %>/admin/adminMemberUpdate";
	frm.attr('action',url);
...
function fn_delete_member(){
	...
	if(bool){
		var frm = $("#memberFrm");
		var url = "<%=request.getContextPath() %>/admin/adminDelete";
		frm.attr('action',url);
		//console.log($("#memberFrm").attr('action'));
		frm.submit();
	}
