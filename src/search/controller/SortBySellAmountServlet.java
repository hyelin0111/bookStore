...
/**
 * Servlet implementation class SortBySellAmount
 */
@WebServlet("/sort/sellAmount")
public class SortBySellAmountServlet extends HttpServlet {
   private static final long serialVersionUID = 1L;

   /**
    * @see HttpServlet#HttpServlet()
    */
   public SortBySellAmountServlet() {
      super();
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      switch (searchType) {
      case "all":
         list = new BookService().selectBookByAllOrderBySellAmount(cPage, numPerPage, searchBook);
         type = "전체";
         totalBoard = new BookService().selectBookByAllCount(searchBook);
         break;
      case "title":
         list = new BookService().selectBookByTitleOrderBySellAmount(cPage, numPerPage, searchBook);
         type = "책제목";
         totalBoard = new BookService().selectBookByTitleCount(searchBook);
         break;
      case "writer":
         list = new BookService().selectBookByWriterOrderBySellAmount(cPage, numPerPage, searchBook);
         type = "저자명";
         totalBoard = new BookService().selectBookByWriterCount(searchBook);
         break;
      case "publisher":
         list = new BookService().selectBookByPublisherOrderBySellAmount(cPage, numPerPage, searchBook);
         type = "출판사명";
      ...
      if (pageNo == 1) {
         ...
         pageBar += "<a href= '" + request.getContextPath() + "/sort/sellAmount?type=" + type
