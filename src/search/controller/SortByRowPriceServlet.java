...
/**
 * Servlet implementation class SortRowPriceServlet
 */
@WebServlet("/sort/rowPrice")
public class SortByRowPriceServlet extends HttpServlet {
   private static final long serialVersionUID = 1L;

   /**
    * @see HttpServlet#HttpServlet()
    */
   public SortByRowPriceServlet() {
      super();
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      switch (searchType) {
      case "all":
         list = new BookService().selectBookByAllOrderByRowPrice(cPage, numPerPage, searchBook);
         type = "전체";
         totalBoard = new BookService().selectBookByAllCount(searchBook);
         break;
      case "title":
         list = new BookService().selectBookByTitleOrderByRowPrice(cPage, numPerPage, searchBook);
         type = "책제목";
         totalBoard = new BookService().selectBookByTitleCount(searchBook);
         break;
      case "writer":
         list = new BookService().selectBookByWriterOrderByRowPrice(cPage, numPerPage, searchBook);
         type = "저자명";
         totalBoard = new BookService().selectBookByWriterCount(searchBook);
         break;
      case "publisher":
         list = new BookService().selectBookByPublisherOrderByRowPrice(cPage, numPerPage, searchBook);
         type = "출판사명";
      ...
      if (pageNo == 1) {
         ...
         pageBar += "<a href= '" + request.getContextPath() + "/sort/rowPrice?type=" + type
