...
/**
 * Servlet implementation class OrderListPageEnd
 */
@WebServlet("/order/orderListPageEnd")
public class OrderListPageEnd extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public OrderListPageEnd() {
		super();
...
//		System.out.println("memberNo="+memberNo+"\t "+startDate+"\t "+endDate+"\t "+"\t "+bookTitle);
		List<Order> list = new OrderService().selectOrderByDate(memberNo, startDate, endDate, bookTitle, cPage, numPerPage);
...
//		int totalPage = (int) Math.ceil((double) totalBookCount / numPerPage);
		...
		int totalBookCount = new OrderService().selectOrderByDateCount(memberNo, startDate, endDate, bookTitle);
		int totalPage = (int) Math.ceil((double) totalBookCount / numPerPage);
